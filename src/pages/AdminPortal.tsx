              ...
              <div className="relative h-40 flex items-end gap-3 pt-4">
                {[
                  ...
                  <div key={d.month} className="flex-1 flex flex-col items-center gap-1">
                    <div className="text-[10px] font-mono text-[#5E7378]">${d.val}K</div>
                    <div
          ...
          <div className="space-y-5">
            <Card>
              ...
              {[
                ...
                { label: "Delayed", count: 1, pct: 9, color: "#DC2626" },
              ].map(s => (
        ...
        <div className="grid lg:grid-cols-2 gap-5">
          ...
          <Card>
            ...
            {[
              ...
              <div key={p.name} className="flex items-center gap-3 py-2.5 border-b border-[rgba(16,27,30,0.05)] last:border-0">
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-medium text-[#33474B] truncate">{p.name}</div>
                  <div className="text-[10px] text-[#7C9096]">{p.client} · {p.pm}</div>
                </div>
                <div className="w-20 hidden sm:block">
                  <ProgressBar value={p.progress} color={p.health === "At Risk" ? "#CA8A04" : "#16A34A"} />
                </div>
          ...
          <Card>
            ...
            {[
              ...
              <div key={s.stage} className="flex items-center gap-3 py-2 border-b border-[rgba(16,27,30,0.05)] last:border-0">
...
function RolesPermissions() {
  const roles = [
    ...
    { name: "Client", users: 28, color: "#4A6064", desc: "Read-only project view" },
  ];
  const perms = [
    ...
    <div className="flex-1 overflow-y-auto p-6 animate-fade-in">
      <div className="max-w-7xl">
        ...
        <div className="grid lg:grid-cols-4 gap-5">
          <div>
            <div className="text-xs font-semibold text-[#4A6064] mb-3 uppercase tracking-wider">Roles</div>
          ...
          <div className="lg:col-span-3">
            <div className="flex items-center justify-between mb-3">
              <div className="text-xs font-semibold text-[#4A6064] uppercase tracking-wider">Permission Matrix — {activeRole}</div>
              <Button size="sm" variant="secondary">Edit Permissions</Button>
            </div>
            <Card>
              {perms.map(perm => (
                <div key={perm.group} className="mb-4 last:mb-0">
                  <div className="text-[10px] font-mono text-[#7C9096] uppercase tracking-widest mb-2">{perm.group}</div>
                  <div className="grid grid-cols-5 gap-2">
                    {perm.items.map((item, i) => {
                      const allowed = matrix[activeRole]?.[perm.group]?.[i] ?? false;
                      return (
                        <div key={item} className={`flex items-center gap-1.5 p-2 rounded-lg border text-[11px] ${allowed ? "bg-[rgba(74,222,128,0.06)] border-[rgba(74,222,128,0.15)] text-[#16A34A]" : "bg-[rgba(16,27,30,0.02)] border-[rgba(16,27,30,0.05)] text-[#A3B4B7]"}`}>
                          <span>{allowed ? "✓" : "✕"}</span>
...
function AIAssistant() {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden animate-fade-in">
      ...
      <div className="border-t border-[rgba(16,27,30,0.07)] p-4">
        <div className="max-w-3xl mx-auto">
          ...
          {team.map(emp => (
            <Card key={emp.name} hover>
              ...
              <div className="flex flex-wrap gap-1.5 mb-4">
                {emp.skills.map(s => <span key={s} className="text-[10px] font-mono text-[#7C9096] bg-[rgba(16,27,30,0.04)] border border-[rgba(16,27,30,0.06)] px-2 py-0.5 rounded-full">{s}</span>)}
              </div>
              <ProgressBar value={emp.workload} label="Workload" showPercent color={emp.workload > 85 ? "#DC2626" : emp.workload > 70 ? "#CA8A04" : "#16A34A"} />
              <div className="text-[11px] text-[#7C9096] mt-2">{emp.projects} active projects</div>
            </Card>
          ))}
        </div>
      </div>
