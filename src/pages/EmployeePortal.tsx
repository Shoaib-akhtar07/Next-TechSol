            ...
            <Card>
              ...
              {[
                ...
                <div key={t.id} className="flex items-center gap-3 py-3 border-b border-[rgba(16,27,30,0.05)] last:border-0">
                  ...
                  <span className="text-[11px] font-mono text-[#7C9096] hidden sm:block">{t.est}</span>
                </div>
              ))}
            </Card>
            ...
            <Card>
              ...
              <div className="grid grid-cols-4 gap-3 mt-4">
                {[
                  ...
                  <div key={s.label} className="text-center bg-[#DCE7E7] rounded-lg p-2">
                    <div className="text-lg font-semibold" style={{ color: s.color }}>{s.count}</div>
                    <div className="text-[10px] text-[#7C9096]">{s.label}</div>
                  </div>
                ))}
              </div>
            </Card>
          ...
          <div className="space-y-5">
            ...
            <Card>
              ...
              {[
                ...
                <div key={t.task} className="flex items-center justify-between py-2 border-b border-[rgba(16,27,30,0.05)] last:border-0">
                  <div>
                    <div className="text-xs text-[#33474B]">{t.task}</div>
                    <div className="text-[10px] font-mono text-[#7C9096]">{t.project}</div>
                  </div>
                  <span className="text-xs font-mono text-[#4A6064]">{t.time}</span>
                </div>
              ))}
...
function SprintBoard() {
  const columns = [
    {
      id: "backlog", label: "Backlog", color: "#7C9096",
      tasks: [
    ...
    {
      id: "todo", label: "To Do", color: "#4A6064",
      tasks: [
    ...
    {
      id: "review", label: "Code Review", color: "#CA8A04",
      tasks: [
  ...
  const priorityColor: Record<string, string> = {
    Critical: "#DC2626", High: "#CA8A04", Medium: "#4A6064", Low: "#7C9096",
  };
        ...
        {columns.map(col => (
          <div key={col.id} className="w-64 flex flex-col">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold text-[#33474B]">{col.label}</span>
              <span className="text-[10px] font-mono text-[#7C9096] ml-auto bg-[rgba(16,27,30,0.05)] px-1.5 py-0.5 rounded-full">{col.tasks.length}</span>
            </div>
            <div className="flex flex-col gap-2">
              {col.tasks.map(task => (
                <div key={task.id} className="bg-[#FFFFFF] border border-[rgba(16,27,30,0.07)] rounded-xl p-3 hover:border-[color-mix(in_srgb,var(--primary)_30%,transparent)] cursor-pointer transition-all group">
                  ...
                  <div className="text-xs font-medium text-[#33474B] leading-relaxed mb-3">{task.title}</div>
                  <div className="flex items-center gap-1.5">
                    <Avatar name={task.assignee} size="xs" />
                    <span className="text-[10px] text-[#7C9096] truncate">{task.assignee.split(" ")[0]}</span>
                  </div>
                </div>
              ))}
