    ...
    <div className="flex items-center gap-2">
      {Array.from({ length: total }, (_, i) => i + 1).map(step => (
        <div key={step} className="flex items-center gap-2">
          <div className={`size-7 rounded-full flex items-center justify-center text-[11px] font-mono font-semibold transition-all ${step < current ? "bg-[#4f6ef7] text-white" : step === current ? "bg-[#4f6ef7] text-white ring-4 ring-[rgba(79,110,247,0.2)]" : "bg-[rgba(255,255,255,0.05)] text-[#5a5a66] border border-[rgba(255,255,255,0.08)]"}`}>
            {step < current ? "✓" : step}
          </div>
          {step < total && <div className={`w-8 h-px transition-colors ${step < current ? "bg-[#4f6ef7]" : "bg-[rgba(255,255,255,0.08)]"}`} />}
        </div>
      ))}
    </div>
  );
...
function Step1({ data, onChange }: { data: string[]; onChange: (types: string[]) => void }) {
  ...
  return (
    <div>
      <div className="text-lg font-semibold text-[#f1f1f3] mb-1">What are you building?</div>
      <div className="text-sm text-[#6b6b7a] mb-6">Select all that apply. You can choose more than one.</div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {types.map(t => (
          <button
            key={t.id}
            onClick={() => toggle(t.id)}
            className={`p-4 rounded-xl border text-left transition-all ${data.includes(t.id) ? "bg-[rgba(79,110,247,0.1)] border-[#4f6ef7] shadow-[0_0_0_1px_rgba(79,110,247,0.4)]" : "bg-[#111118] border-[rgba(255,255,255,0.07)] hover:border-[rgba(79,110,247,0.25)] hover:bg-[rgba(79,110,247,0.03)]"}`}
          >
            <div className={`text-xl mb-2 ${data.includes(t.id) ? "text-[#4f6ef7]" : "text-[#5a5a66]"}`}>{t.icon}</div>
            <div className={`text-xs font-medium ${data.includes(t.id) ? "text-[#f1f1f3]" : "text-[#9494a0]"}`}>{t.label}</div>
          </button>
        ))}
      </div>
    </div>
...
function Step2({ data, onChange }: { data: Partial<FormData>; onChange: (d: Partial<FormData>) => void }) {
  return (
    <div>
      <div className="text-lg font-semibold text-[#f1f1f3] mb-1">Tell us about your business</div>
      <div className="text-sm text-[#6b6b7a] mb-6">This helps us match you with the right engineering team.</div>
...
function Step3({ data, onChange }: { data: Partial<FormData>; onChange: (d: Partial<FormData>) => void }) {
  return (
    <div>
      <div className="text-lg font-semibold text-[#f1f1f3] mb-1">Project Requirements</div>
      <div className="text-sm text-[#6b6b7a] mb-6">Describe your project in as much detail as you can. Include current challenges, desired outcomes, and any technical requirements.</div>
      <Textarea
        ...
        rows={10}
      />
      <div className="mt-4 p-4 bg-[rgba(79,110,247,0.05)] border border-[rgba(79,110,247,0.15)] rounded-xl">
        <div className="text-xs font-semibold text-[#4f6ef7] mb-2">💡 Tips for a better response</div>
        <div className="grid sm:grid-cols-2 gap-1.5">
          {["Describe the business problem you're solving", "Mention any existing systems to integrate with", "Share your technical preferences if any", "Describe your target users"].map(tip => (
            <div key={tip} className="text-[11px] text-[#6b6b7a] flex items-start gap-1.5">
              <span className="text-[#4f6ef7]">→</span> {tip}
            </div>
          ))}
        </div>
      </div>
...
function Step4({ data, onChange }: { data: Partial<FormData>; onChange: (d: Partial<FormData>) => void }) {
  ...
  return (
    <div>
      <div className="text-lg font-semibold text-[#f1f1f3] mb-1">Project Budget</div>
      <div className="text-sm text-[#6b6b7a] mb-6">Select the range that best fits your budget. This helps us scope the project correctly.</div>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {ranges.map(r => (
          <button
            key={r.id}
            onClick={() => onChange({ ...data, budget: r.id })}
            className={`p-4 rounded-xl border text-left transition-all ${data.budget === r.id ? "bg-[rgba(79,110,247,0.1)] border-[#4f6ef7]" : "bg-[#111118] border-[rgba(255,255,255,0.07)] hover:border-[rgba(79,110,247,0.25)]"}`}
          >
            <div className={`text-sm font-semibold mb-1 ${data.budget === r.id ? "text-[#f1f1f3]" : "text-[#c4c4cc]"}`}>{r.label}</div>
            <div className="text-[11px] text-[#5a5a66]">{r.sub}</div>
          </button>
        ))}
      </div>
    </div>
...
function Step5({ data, onChange }: { data: Partial<FormData>; onChange: (d: Partial<FormData>) => void }) {
  ...
  return (
    <div>
      <div className="text-lg font-semibold text-[#f1f1f3] mb-1">Desired Timeline</div>
      <div className="text-sm text-[#6b6b7a] mb-6">When would you like to start, and what's your expected delivery window?</div>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {options.map(o => (
          <button
            key={o.id}
            onClick={() => onChange({ ...data, timeline: o.id })}
            className={`p-4 rounded-xl border text-left transition-all ${data.timeline === o.id ? "bg-[rgba(79,110,247,0.1)] border-[#4f6ef7]" : "bg-[#111118] border-[rgba(255,255,255,0.07)] hover:border-[rgba(79,110,247,0.25)]"}`}
          >
            <div className={`text-sm font-semibold mb-1 ${data.timeline === o.id ? "text-[#f1f1f3]" : "text-[#c4c4cc]"}`}>{o.label}</div>
            <div className="text-[11px] text-[#5a5a66]">{o.sub}</div>
          </button>
        ))}
      </div>
    </div>
...
function Step6() {
  ...
  return (
    <div>
      <div className="text-lg font-semibold text-[#f1f1f3] mb-1">Attachments</div>
      <div className="text-sm text-[#6b6b7a] mb-6">Upload any relevant documents — briefs, specs, designs, or existing materials.</div>
      <div
        ...
        className={`border-2 border-dashed rounded-xl p-10 text-center transition-all ${dragging ? "border-[#4f6ef7] bg-[rgba(79,110,247,0.06)]" : "border-[rgba(255,255,255,0.1)] hover:border-[rgba(79,110,247,0.3)] bg-[rgba(255,255,255,0.02)]"}`}
      >
        <div className="text-3xl mb-3 opacity-40">⬡</div>
        <div className="text-sm font-medium text-[#c4c4cc] mb-1">Drag & drop files here</div>
        <div className="text-xs text-[#5a5a66] mb-4">PDF, DOCX, images, ZIP — Max 50MB per file</div>
        <Button variant="secondary" size="sm">Browse Files</Button>
      </div>
      {files.length > 0 && (
        <div className="mt-4 space-y-2">
          {files.map(f => (
            <div key={f} className="flex items-center gap-3 p-3 bg-[#111118] border border-[rgba(255,255,255,0.07)] rounded-lg">
              <span className="text-sm">📄</span>
              <span className="text-xs text-[#c4c4cc] flex-1">{f}</span>
              <button className="text-[#5a5a66] hover:text-[#f87171] transition-colors" onClick={() => setFiles(prev => prev.filter(x => x !== f))}>✕</button>
            </div>
          ))}
        </div>
      )}
      <div className="mt-4 p-3 bg-[rgba(250,204,21,0.05)] border border-[rgba(250,204,21,0.15)] rounded-xl">
        <div className="text-[11px] text-[#c4c4cc] flex items-start gap-2">
...
function Step7({ data }: { data: FormData }) {
  ...
  return (
    <div>
      <div className="text-lg font-semibold text-[#f1f1f3] mb-1">Review Your Submission</div>
      <div className="text-sm text-[#6b6b7a] mb-6">Please review your project details before submitting.</div>
      <div className="space-y-4">
        {[
          ...
          <div key={item.label} className="flex gap-4 py-3 border-b border-[rgba(255,255,255,0.05)]">
            <div className="w-32 flex-shrink-0 text-xs text-[#5a5a66]">{item.label}</div>
            <div className="text-xs text-[#c4c4cc] flex-1">{item.value}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-8">
      <div className="max-w-md text-center animate-fade-in">
        ...
        <h2 className="text-2xl font-semibold text-[#f1f1f3] mb-2">Project Request Received</h2>
        <p className="text-sm text-[#6b6b7a] mb-6">Our engineering team will review your submission and respond within 48 hours with a detailed proposal.</p>
        <div className="bg-[#111118] border border-[rgba(255,255,255,0.07)] rounded-xl p-4 mb-8">
          <div className="text-xs text-[#5a5a66] mb-1">Your Reference Number</div>
          <div className="text-xl font-mono font-semibold text-[#4f6ef7]">{refNo}</div>
          <div className="text-[11px] text-[#5a5a66] mt-1">Keep this for your records</div>
        </div>
...
export default function ProjectInquiry({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-[#09090c] flex flex-col">
      {/* Header */}
      <div className="border-b border-[rgba(255,255,255,0.07)] px-6 h-14 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-[#f1f1f3]">Start a Project</span>
        </div>
        <button onClick={onClose} className="text-[#5a5a66] hover:text-[#f1f1f3] transition-colors w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[rgba(255,255,255,0.05)]">✕</button>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center py-10 px-6">
        <div className="w-full max-w-2xl">
          ...
          <div className="flex items-center justify-between mb-10">
            ...
            <div className="text-xs text-[#5a5a66] font-mono">Step {step} of {STEPS.length}</div>
          </div>
          ...
          {/* Navigation */}
          <div className="flex items-center justify-between mt-10 pt-6 border-t border-[rgba(255,255,255,0.07)]">
